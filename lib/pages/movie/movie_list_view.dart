...
class _MovieListViewState extends State<MovieListView> {
  ...
  Future<void> fetchData() async {
    ...
    switch (action) {
      case 'in_theaters':
       String jsonStr = await client.getNowPlayingList(start: start, count: count);
       jsonData=json.decode(jsonStr);
       break;
      case 'coming_soon':
       String jsonStr = await client.getComingPlayList(start: start, count: count);
       jsonData=json.decode(jsonStr);
        break;
      case 'search':
//        data = await client.getSearchListByTag(tag: title, start: start, count: count);
    ...
    setState(() {
      ...
      List<MovieItem> newMovies = getMovieList(jsonData['subjects']);
